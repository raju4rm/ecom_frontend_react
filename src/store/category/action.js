...
export const setEdit = (payload) => async (dispatch) => {  
  dispatch(loadingStart());  
  try {  
      const response = await axios.put('admin/master/category',payload);  
      const {data} = response;
      if (data.status) {
        dispatch(successStatus(data)); 
      }   
...
export const getEdit = (payload) => async (dispatch) => {   
  try {  
    let baseUrl     = `admin/master/category/edit/${payload}` 
...
export const getList = (payload) => async (dispatch) => {    
  try {
    let baseUrl     = `admin/master/category?search=1&page=${payload.pageNo}&per_page=${payload.limitPerPage}` 
    const {data} = await axios.get(baseUrl);  
    if (data.status) {
