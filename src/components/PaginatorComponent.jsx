...
const Pagiationcomp= ({ totalRecords })=> {
    ...
    const onPageChange = (e, index) => {
        ...
        dispatch(setPaginationData({ page:(e.page + 1), limit:(e.rows) })) 
    };
    ...
    const prevNewPageNoRef = useRef();

    useEffect(() => {
      if (prevNewPageNoRef.current !== newPageNo) {
        dispatch(setPaginationData({ page: newPageNo, limit: limitPerPage }));
        setParams((prevState) => ({ ...prevState, page: newPageNo, limit: limitPerPage }));
        prevNewPageNoRef.current = newPageNo; // Update the ref to the current newPageNo
      } else {
        dispatch(setPaginationData({ page: pageNo, limit: limitPerPage }));
        setParams((prevState) => ({ ...prevState, page: pageNo, limit: limitPerPage }));
      }
    }, [limitPerPage, pageNo, newPageNo]);
   ...
   /*  const template2 = {
    ...
    return (
        <div className="card">
            ...
            <Divider />
            {/* <Paginator template={template2} first={first[1]} rows={rows[1]} totalRecords={totalRecords} onPageChange={(e) => onPageChange(e, 1)} className="justify-content-end" />
            <Divider />
            <Paginator template={template3} first={first[2]} rows={rows[2]} totalRecords={totalRecords} onPageChange={(e) => onPageChange(e, 2)} className="justify-content-start" /> */}
        </div>
    )
}


const PaginatorComponent = React.memo(Pagiationcomp);
