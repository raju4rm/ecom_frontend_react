...
import { store } from '../store/reduxPersist';
import { setAccessToken, logout } from '../store/login/slice';
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const newAccessToken = refreshResponse.data.data[0].token;
        store.dispatch(setAccessToken(refreshResponse.data));
